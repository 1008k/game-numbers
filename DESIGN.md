...
- Primary user goal: 限られた数字パネルを配分し、すべてのターゲット数字を成立させる
- Business objective: 広告付きスマホカジュアルゲームとして継続プレイされる体験を作る
- Desired user feeling: すぐ分かる、気持ちよく押せる、解けるとすっきりする
- Design maturity: draft

## 2. Design Direction
- Overall tone: 電卓風で明快、軽快、カジュアル
- Visual keywords: LED表示、数字タイル、押しやすいパネル、強い正解演出、縦画面
- Interaction keywords: タップだけ、即時反応、パネル吸着、短いステージ、リトライしやすい
- Reference products: 電卓UI、数字タイルパズル、短時間スマホパズル
- Avoid: 計算ドリル感、長い説明文、細かすぎる入力、反応の遅い演出
...
- Never change without updating this file: 電卓風の中心トーン、タップだけで遊ぶ前提、縦固定の画面構成
- Out of scope for design agents: ランキング、デイリー、大量スキン、課金画面の詳細

## 4. Experience Principles
- Primary screen goal: 選択中ターゲットに対して、今あるパネルで式を作る
- Information priority: 残りミス、ターゲット、現在の式、使用可能パネル、補助操作の順に分かる
- Navigation principle: 1ステージ中はゲーム画面に集中し、ホームや設定への導線は控えめにする
- Feedback principle: タップ、式完成、正解、ミス、パネル消費を即時に見せる
- Error recovery principle: ミス後も選択状態を理解しやすくし、リトライや広告復活に迷わせない

## 5. Visual System

### Color
- Primary: 電卓キーを思わせる明るいアクセント色
- Secondary: 演算子や補助操作用の落ち着いたアクセント色
- Background: 数字が読みやすい低ノイズ背景
- Surface: パネルと表示領域が明確に分かれる面
- Text: 数字の視認性を最優先
- Border: パネルの押せる領域が分かる程度
- Success: 正解時に強く光る色
...
- Number/data style: LED風または電卓表示風の強い数字表現

### Spacing
- Base spacing: 親指で押しやすい間隔
- Section spacing: ターゲット、式、パネルを明確に分ける
- Dense areas: パネルグリッド
...
- Button shape: パネルとボタンの役割が区別できる形
- Input shape: 数字入力欄ではなく、現在の式表示として扱う

### Elevation
- Default: パネルの押せる感じが分かる軽い奥行き
...
- Consumed panel: 消費済みであることを明確にし、他ターゲットで再利用できないことを伝える

### Status Displays
- Targets: 未クリア、選択中、クリア済みを一目で区別する
- Expression: `number operator number` の組み立て状態を中央で見せる
...
### Game
- User goal: パネルを選んでターゲットを成立させる
- Primary action: パネル選択
- Key information: ステージ情報、残りミス、ターゲット、現在の式、使用可能パネル
- Layout: 上からステージ情報、ターゲット、式、パネル、補助操作
- Error state: ミス理由を短く表示し、次の操作を妨げない
- Success state: ターゲットクリアとパネル消費を明確に見せる
...
## 8. Responsive Rules
- Mobile: 縦固定を前提に、親指で押しやすいパネルサイズを優先する
...
- Error copy: `計算が違います`, `0で割れません`, `パネルが足りません` のように短くする
...
Primary action: tap number and operator panels to complete target numbers
Desired feeling: calculator-like, snappy, satisfying, easy to understand
Visual direction: LED display, tactile number panels, vertical mobile layout
References: calculator UI, short-session number tile puzzle games
Avoid: worksheet-like math drill, manual numeric input, long explanations
Important states: selected target, expression building, consumed panel, correct answer, mistake, game over, rewarded revival
```

## 11. Review Checklist
- The game screen makes the current target and available panels obvious.
- The expression state is readable at a glance.
- Consumed panels cannot be mistaken for usable panels.
