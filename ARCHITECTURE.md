...
この文書は、`ナンバータイル` の実装構造を素早く把握する補助文書です。仕様の正本は `docs/project-spec.md`、実装判断は `docs/rules-coding.md` を参照します。

現時点では手作りステージを5つ遊べる試作を実装中です。ここでは現在のGodotシーン、スクリプト、責務分割を記載します。
...
- `GameScreen.tscn`: ステージ情報、ゴール、現在の式、タイル、補助操作を表示する中心画面。
- `PanelButton.tscn`: 数字タイルと記号タイルの共通表示部品。
- `TargetItem.tscn`: ゴール数字の表示、選択中、クリア済み状態。
...
- `GameManager.gd`: ステージ進行、ゴール選択、式入力状態、ミス、クリア、ゲームオーバーを管理する。
- `StageGenerator.gd`: 初回は手作りステージを返し、将来は先に解答を作る生成へ拡張する。
- `ExpressionEvaluator.gd`: `number operator number` の式評価、0除算、不成立判定を扱う。
- `AdManager.gd`: 現在はリワード広告復活のスタブを扱う。
- `SaveManager.gd`: 現在は空実装に近い進捗保存スタブを扱う。
- `SoundManager.gd`: 現在は音再生のスタブを扱う。

## 4. 責務分割
- 中心ロジックはUI、広告SDK、保存形式に依存させない。
- `StageGenerator.gd` はステージ生成だけを担い、画面表示や広告判断を持たない。
- `ExpressionEvaluator.gd` は式評価だけを担い、タイル消費やスコア計算を持たない。
...
## 7. ビルド / テスト / デプロイ
- ビルド方法: 未定義。Godotのエクスポート設定を追加した段階でREADMEへ追記する。
