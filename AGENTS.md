...
- 繰り返し作業や公開運用では、`CI`、`GitHub Actions`、スクリプト、既存の便利な仕組みで安定化できるものを前向きに検討します。
- 軽微な修正では `track` を必須にせず、仕様、設計、運用、公開物に影響する大きめの変更では `docs/tracks/` に作業文脈を残します。
- 変更履歴として共有したい内容は `CHANGELOG.md` に記載し、必要なら対応する `track` を参照します。
